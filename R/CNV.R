...
CNV <- function(file = NA, Chromosome = NA,mutantname = NA, controlname = NA,size=NA,alpha=NA,color=NA){
  ...
  r <- r +  ggplot2::geom_point(ggplot2::aes_string(x=table$Strt, y = y), color="purple") + ggplot2::geom_point(ggplot2::aes_string(x=table$Strt, y = z), color = color,size = size,alpha=alpha) + facet_grid(~Chrom) + ggplot2::ggtitle("Copy Number Variation Control and Mutant Sample") + ylab("CNV Ratio") + xlab("Position")
