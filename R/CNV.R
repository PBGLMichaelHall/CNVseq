...
CNV <- function(file = NA, Chromosome = NA,mutantname = NA, controlname = NA,size=NA,alpha=N){
  ...
  p <- p + ggplot2::geom_point(ggplot2::aes_string(x=table$Strt, y = y), color="red") + ggplot2::geom_point(ggplot2::aes_string(x=table$Strt, y = z), color = "green",size = size,alpha=alpha) + facet_grid(~Chrom) + ggplot2::ggtitle("Copy Number Variation Control Sample") + ylab("CNV Ratio") + xlab("Position")
